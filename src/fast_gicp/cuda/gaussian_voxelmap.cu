...
namespace {
  ...
  struct voxel_normalization_kernel {
    ...
    GaussianVoxelMap::GaussianVoxelMap(float resolution, int init_num_buckets, int max_bucket_scan_count) : init_num_buckets(init_num_buckets), max_bucket_scan_count(max_bucket_scan_count), voxel_resolution(resolution) {}
    ...
    void GaussianVoxelMap::create_hashtable(const thrust::device_vector<Eigen::Vector3i>& voxel_coords) {
    ...
    const thrust::pair<Eigen::Vector3i, int>* GaussianVoxelMap::find(const Eigen::Vector3i& coord, const thrust::host_vector<thrust::pair<Eigen::Vector3i, int>>& buckets) const {
      uint64_t hash = vector3i_hash(coord);
      for(int i = 0; i < max_bucket_scan_count; i++) {
    ...
    bool GaussianVoxelMap::insert(const thrust::pair<Eigen::Vector3i, int>& voxel, thrust::host_vector<thrust::pair<Eigen::Vector3i, int>>& buckets) {
    ...
    void GaussianVoxelMap::rehash(thrust::host_vector<thrust::pair<Eigen::Vector3i, int>>& buckets) {
